...
def _clean_inputs(lp):
    ...
    try:
        ...
        if False:
        #if (False and sps.issparse(A_ub) and not np.isfinite(A_ub.data).all()
    ...
    try:
        ...
        if False:# and sps.issparse(A_eq) and not np.isfinite(A_eq.data).all()
...
def _presolve(lp, rr, rr_method, tol=1e-9):
    ...
    if False and sps.issparse(A_eq):
        A_eq = A_eq.tocsr()
        A_ub = A_ub.tocsr()
...
def _get_Abc(lp, c0):
    ...
    print("****************")
    print("_get_Abc() return values:")
    print("A = ")
    print(A)
    print("b = ", b)
    print("c = ", c)

    print("ending _get_Abc()")
    print("=========================================================")
    #exit(0)
...
def _postsolve(x, postsolve_args, complete=False):
    ...
    print("x before _unscale = ", x)
    x = _unscale(x, C, b_scale)
    print("x after  _unscale = ", x)
